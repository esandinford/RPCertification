<template>
	<view>
		<!-- 要嵌套的网页放在这里面 src可是是本地路径也可以是云端路径 使用云端路径时要配置origin 本地html文件要放在 hybrid/html里面 -->
		<web-view :webview-styles="webviewStyles" @message="handleMessage" src="/hybrid/html/index.html"></web-view>
		<form>
			<view align="center"><image class="esand_logo" src="../../static/logo.png"></image></view>
			<view class="infos">
			...
			<view align="center"><button class="btSubmit" @click="formSubmit()">实人认证</button></view>
		</form>
		<!-- <div align="center"><textarea v-model="msg" /></div> -->
...
export default {
	...
	methods: {
		...
		//验证结束跳转网页并且显示网页
		VerifyEnd:function(e){
			//跳转网页
			wv.loadURL("/hybrid/html/result.html?"+e);
			//显示webview
			wv.show();
		},
			...
			if (zolozResult.code == 'ZOLOZ_SUCCESS') {
				...
				uni.request({
					...
					success: res => {
						...
						// 发起认证操作
						ZolozModule.zolozAuth({ initMsg: res.data }, ret => {
							...
							if (ret.code == 'ZOLOZ_SUCCESS') {
								...
								uni.request({
									...
									success: res => {
										...
										//验证发生错误时跳转网页
										_this.VerifyEnd("code=200&msg=success&details="+JSON.stringify(res.data));
									}
								});
							} else {
								...
								//验证发生错误时跳转网页
								_this.VerifyEnd("code=-1&msg=verifyError&details="+JSON.stringify(ret));
							}
						});
					},
					fail:e=>{
						//验证发生错误时跳转网页
						_this.VerifyEnd("code=-1&msg=netError&details="+JSON.stringify(e));
					}
				});
				...
				//验证发生错误时跳转网页
				_this.VerifyEnd("code=-1&msg=initError&details="+JSON.stringify(zolozResult));
			}
			//_this.VerifyEnd(" hello=1&x=2");
		}
	}
};
</script>
